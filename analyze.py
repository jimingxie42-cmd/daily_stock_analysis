...
## 三、短线操作建议
| 股票 | 评分(0-100) | 操作 | 建议仓位 | 入场/加仓位 | 止盈位 | 止损位 | 核心理由 |
|------|:---:|------|:---:|------|------|------|------|
（评分维度：趋势强度30 + 量价配合30 + 位置安全20 + 催化驱动20。必须基于用户实际持仓成本。）

## 四、情景应对
（每只股票：如果涨→怎么做 | 如果横→怎么做 | 如果跌→怎么做。具体价格触发条件。）
...
请严格按系统指令中的八段结构输出短线操作指导。针对每个时段(现在是北京时间{time.strftime('%H:%M')})，侧重该时段的输出重点。"""
